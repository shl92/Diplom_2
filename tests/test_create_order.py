from data import ENDPOINTS, TextMessage, Order, URLS
import pytest
import allure
...
class TestCreateOrder:
    ...
    @allure.description("Запросы с авторизацией с валидным хэшем ингридиентов и без передачи ингридиентов, "
                        "ожидаем успешный статус ответа (200) / ошибку клиента (400) и текста ответа 'True' / 'False'")
    ...
    def test_create_order_authorized_with_or_no_ingredients(self, create_new_user_with_delete, ingredients, status_code,
        ...
        response = BaseApiMethods.post_request(URLS.MAIN_PAGE_URL + ENDPOINTS.CREATE_OR_GET_ORDER, payload_order,
    ...
    @allure.description("Запросы без авторизации с валидным хэшем ингридиентов и без передачи ингридиентов, "
                        "ожидаем успешный статус ответа (200) / ошибку клиента (400) и текста ответа 'True' / 'False'")
    @pytest.mark.parametrize('ingredients, status_code, text_answer',
                             [(Order.INGREDIENTS, StatusCode.CODE_200, TextMessage.SUCCESS_TEXT),
                              (Order.INGREDIENTS_EMPTY, StatusCode.CODE_400, TextMessage.FALSE_TEXT)],
                             ids=["with_ingredients", "without_ingredients"])
    def test_create_order_unauthorized_with_or_no_ingredients(self, create_new_user_with_delete, ingredients,
                                                              status_code, text_answer):
        payload_order = {"ingredients": ingredients}
        response = BaseApiMethods.post_request(URLS.MAIN_PAGE_URL + ENDPOINTS.CREATE_OR_GET_ORDER, payload_order)
        CheckMethods.check_status_code(response, status_code)
        CheckMethods.check_json_message(response, TextMessage.SUCCESS_KEY, text_answer)
    ...
    @allure.description("Запрос с авторизацией с невалидным (несуществующим) хэшем ингридиентов, ожидаем ошибку "
                        "сервера (500), текст ответа 'Internal Server Error'")
    ...
    @allure.description("Запрос без авторизации с невалидным (несуществующим) хэшем ингридиентов, ожидаем ошибку "
                        "сервера (500), текст ответа 'Internal Server Error'")
